    ...
    pub fn assemble(program: &Vec<Operator>) -> Vec<u8> {
        ...
        for operation in program {
            match operation {
                ...
                /*opcode : 8*/                Operator::CONST_S(v1, v2) => {
                ...
                /*opcode :20*/                Operator::JMP_DEF(v1, v2) => {
                    let mut op_bytes: Vec<u8> = vec![0x14];
                    op_bytes.extend_from_slice(&*str_op_value_bytes(&v1));
                    op_bytes.extend_from_slice(&v2.to_be_bytes());
                    val.extend_from_slice(&op_bytes);
                }
                /*opcode :21*/                Operator::LABEL(v) => {
                ...
                /*opcode :22*/                Operator::SYSCALL(v1, v2) => {
                ...
                /*opcode :23*/                Operator::SYSCALLD(v) => {
                    let mut op_bytes: Vec<u8> = vec![0x17];
                    op_bytes.extend_from_slice(&v.to_be_bytes());

                    val.extend_from_slice(&op_bytes);
                }
                /*opcode :24*/                Operator::EXCEPT_THROW => {
                    val.push(0x18);
                }
                /*opcode :25*/                Operator::EXCEPT_CATCH(v) => {
                    let mut op_bytes: Vec<u8> = vec![0x19];
                    op_bytes.extend_from_slice(&*str_op_value_bytes(&v));

                    val.extend_from_slice(&op_bytes);
                }
                /*opcode :26*/                Operator::RET => {
                    val.push(0x1A);
                }
                /*opcode :27*/                Operator::EMIT => {
                    val.push(0x1b);
                }
                /*opcode :28*/                Operator::EMITS(v) => {
                    let mut op_bytes: Vec<u8> = vec![0x1C];
                    op_bytes.extend_from_slice(&v.to_be_bytes());

                    val.extend_from_slice(&op_bytes);
                }
                /*opcode :29*/                Operator::EMITW(v) => {
                    let mut op_bytes: Vec<u8> = vec![0x1D];
                    op_bytes.extend_from_slice(&v.to_be_bytes());

                    val.extend_from_slice(&op_bytes);
                }
                /*opcode :30*/                Operator::EMITD(v) => {
                    let mut op_bytes: Vec<u8> = vec![0x1E];
                    op_bytes.extend_from_slice(&v.to_be_bytes())
                }
                /*opcode :31*/                Operator::GETBYTELEN(v) => {
                    let mut op_bytes: Vec<u8> = vec![0x1F];
                    op_bytes.extend_from_slice(&v.to_be_bytes());

                    val.extend_from_slice(&op_bytes);
                }
                /*opcode :32*/                Operator::GETBYTE(v1, v2) => {
                ...
                /*opcode :33*/                Operator::GETWORD(v1, v2) => {
                ...
                /*opcode :34*/                Operator::SETBYTE(v1, v2, v3) => {
                ...
                /*opcode :35*/                Operator::SETWORD(v1, v2, v3) => {
                ...
                /*opcode :36*/                Operator::DUPO(v) => {
                    let mut op_bytes: Vec<u8> = vec![0x24];
                    op_bytes.extend_from_slice(&v.to_be_bytes());
                    val.extend_from_slice(&op_bytes);
                }
                /*opcode :37*/                Operator::DUP => {
                    val.push(0x25)
                }
                /*opcode :38*/                Operator::SWAP => {
                    val.push(0x26)
                }
                /*opcode :39*/                Operator::ADDu => {
                    val.push(0x27)
                }
                /*opcode :40*/                Operator::ADDi => {
                    val.push(0x28)
                }
                /*opcode :41*/                Operator::ADDfi => {
                    val.push(0x29)
                }
                /*opcode :42*/                Operator::ADDif => {
                    val.push(0x2A)
                }
                /*opcode :43*/                Operator::ADDf => {
                    val.push(0x2B)
                }
                /*opcode :44*/                Operator::SUBu => {
                    val.push(0x2C)
                }
                /*opcode :45*/                Operator::SUBi => {
                    val.push(0x2D)
                }
                /*opcode :46*/                Operator::SUBfi => {
                    val.push(0x2E)
                }
                /*opcode :47*/                Operator::SUBif => {
                    val.push(0x2F)
                }
                /*opcode :48*/                Operator::SUBf => {
                    val.push(0x30)
                }
                /*opcode :49*/                Operator::MULu => {
                    val.push(0x31)
                }
                /*opcode :50*/                Operator::MULi => {
                    val.push(0x32)
                }
                /*opcode :51*/                Operator::MULfi => {
                    val.push(0x33)
                }
                /*opcode :52*/                Operator::MULif => {
                    val.push(0x34)
                }
                /*opcode :53*/                Operator::MULf => {
                    val.push(0x35)
                }
                /*opcode :54*/                Operator::DIVu => {
                    val.push(0x36)
                }
                /*opcode :55*/                Operator::DIVi => {
                    val.push(0x37)
                }
                /*opcode :56*/                Operator::DIVfi => {
                    val.push(0x38)
                }
                /*opcode :57*/                Operator::DIVif => {
                    val.push(0x39)
                }
                /*opcode :58*/                Operator::DIVf => {
                    val.push(0x3A)
                }
                /*opcode :59*/                Operator::MODu => {
                    val.push(0x3B)
                }
                /*opcode :60*/                Operator::MODi => {
                    val.push(0x3C)
                }
                /*opcode :61*/                Operator::MODfi => {
                    val.push(0x3D)
                }
                /*opcode :62*/                Operator::MODif => {
                    val.push(0x3E)
                }
                /*opcode :63*/                Operator::MODf => {
                    val.push(0x3F)
                }
                /*opcode :64*/                Operator::ROR => {
                    val.push(0x40)
                }
                /*opcode :65*/                Operator::ROL => {
                    val.push(0x41)
                }
                /*opcode :66*/                Operator::LSR => {
                    val.push(0x42)
                }
                /*opcode :67*/                Operator::ASR => {
                    val.push(0x43)
                }
                /*opcode :68*/                Operator::LSL => {
                    val.push(0x44)
                }
                /*opcode :69*/                Operator::ASL => {
                    val.push(0x45)
                }
                /*opcode :70*/                Operator::NEG => {
                    val.push(0x46)
                }
                /*opcode :71*/                Operator::AND => {
                    val.push(0x47)
                }
                /*opcode :72*/                Operator::XOR => {
                    val.push(0x48)
                }
                /*opcode :73*/                Operator::NAND => {
                    val.push(0x49)
                }
                /*opcode :74*/                Operator::CNT => {
                    val.push(0x4A)
                }
                /*opcode :75*/                Operator::CMP => {
                    val.push(0x4B)
                }
                /*opcode :76*/                Operator::JMP_SCAN => {
                    val.push(0x4C)
                }
            }
        }
        ...
        for _ in 4..n_bytes-1 {
            ...
            match byte {
                ...
                0x08 => {
                    ...
                    i += str_len_read;
                    let (usize_value_2, bytes_read_2) = read_next_string(&program_binary, i, string_length);
                    i += bytes_read_2;
                ...
                0x0f => {
                    let (string_length, str_len_read) = read_next_usize(&program_binary, i);
                    i += str_len_read;
                    let (string, bytes_read_2) = read_next_string(&program_binary, i, string_length);
                    i += bytes_read_2;
                ...
                0x13 => {
                    ...
                    i += str_len_read2;
                    let (string2, bytes_read_22) = read_next_string(&program_binary, i, string_length2);
                    i += bytes_read_22;
